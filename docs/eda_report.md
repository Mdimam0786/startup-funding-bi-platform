...
Every number below came out of `src/eda/compute_eda_stats.py` running against the actual warehouse tables — nothing here is estimated. I numbered each insight so I can point back to it, and attached a one-line interpretation instead of just dropping the statistic.

**Scope reminder:** this is a historical Crunchbase snapshot with funding activity concentrated between 1995 and 2015. Any "trend" language below describes that window, not the present-day market.
...
*All 101 numbered insights above came out of the actual warehouse tables via `src/eda/compute_eda_stats.py` — none of it is estimated. Re-running that script reproduces every statistic cited here.*
